# Software-Engineering

## Books

- [代码大全 2nd PDF]({{ files_server }}/current/代码大全%202ND.pdf)
- [代码整洁之道 EPUB]({{ files_server }}/current/代码整洁之道.epub)
- [重构-改善既有代码的设计 PDF]({{ files_server }}/current/重构-改善既有代码的设计.pdf)

## Desgin-Patterns

- [Design-Patterns](https://github.com/microwind/design-patterns)
- [大话设计模式 EPUB]({{ files_server }}/current/design-patterns/大话设计模式.epub)
- [设计模式-可复用面向对象软件的基础 PDF]({{ files_server }}/current/design-patterns/设计模式-可复用面向对象软件的基础.pdf)

------

## Masters

- [Free software, Free society PDF]({{ files_server }}/current/free-software-free-society.pdf)
- [Just for fun](https://github.com/elodiagit/module2-solution/blob/master/Just_For_Fun-Linus_Torvalds%E8%87%AA%E4%BC%A0_%E4%B8%AD%E6%96%87%E7%89%88.pdf)
- [Fabrice Bellard's Home Page](https://www.bellard.org/)
- [DHH](https://dhh.dk/)
- [Mitchell Hashimoto](https://mitchellh.com/)

------

## Extended Books

- [主权个人 PDF]({{ files_server }}/current/the-sovereign-individual.pdf)
- [信息简史 EPUB ]({{ files_server }}/current/%E4%BF%A1%E6%81%AF%E7%AE%80%E5%8F%B2.epub)
- [神经漫游者 EPUB]({{ files_server }}/current/%E7%A5%9E%E7%BB%8F%E6%BC%AB%E6%B8%B8%E8%80%85.epub)
- [雪崩 EPUB]({{ files_server }}/current/%E9%9B%AA%E5%B4%A9.epub)
- [硅谷之火](https://weread.qq.com/web/reader/7bc32df071c955fe7bc0637)



This website was created 🖨 by AvenMay and supported 👍 by Naomi.  
AvenMay ❤️ Naomi, cause Naomi is cute🐰!